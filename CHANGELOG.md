...
- Version 4.106.110 - 28 July 2026
  ...
  - Completed the scheduled documentation-backed retrospective review of versions 4.106.109 and 4.106.110 across endpoint completeness, authentication boundaries, wire types, nullable/empty numeric handling, timestamp units, validation, fixtures, examples, release notes, and execution scope. No runtime or API-contract defect was found; the missing binary-compatibility disclosure for the v4.106.109 liquidation overload signatures was corrected below.
  - Source: https://www.gate.com/docs/developers/apiv4/en/#changelog
...
- Version 4.106.109 - 22 July 2026
  ...
  - Fully aligned public `GET /futures/{settle}/liq_orders` with the current endpoint and `FuturesLiqOrder` contracts.
    ...
    - Preserved omission of every optional query filter, including `contract` and `limit`, through both the request object and convenience overload. This changed the `limit` parameter on both public `GetLiquidationsAsync` convenience overloads from `int` with a default of `100` to nullable `int?` with a default of `null`; the method-signature change is binary-breaking. Added preflight validation for reversed ranges and the documented maximum 3600-second interval.
