...
public class P2pContractTests
{
    ...
    public void Documented_advertisement_and_chat_responses_deserialize()
    {
        ...
        Assert.Equal(2124000001, advertisement.OrderId);
        Assert.Equal(GateP2pOrderSide.Sell, advertisement.Type);
        Assert.Equal(0.908m, advertisement.Rate);
        Assert.Equal("USDT", advertisement.CurrencyType);
        Assert.Equal(9.08m, advertisement.MinimumFiatAmount);
        Assert.Equal(454m, advertisement.MaximumFiatAmount);
        Assert.Equal(GateP2pAdLimitBasis.Crypto, advertisement.LimitBasis);
        Assert.Equal("crypto", advertisement.LimitBasisText);
        Assert.Equal(0, advertisement.PolymarketLimit);
        Assert.Equal(20, advertisement.ExpireMinutes);
        Assert.Equal(-1, advertisement.UserOrdersLimit);
        ...
        Assert.Equal(0.800m, marketAdvertisements[0].Price);
        Assert.Equal(4975.5m, marketAdvertisements[0].SurplusAmount);
        Assert.Equal(0.80m, marketAdvertisements[0].MinimumFiatAmount);
        Assert.Equal(3980.40m, marketAdvertisements[0].MaximumFiatAmount);
        Assert.Equal(GateP2pAdLimitBasis.Crypto, marketAdvertisements[0].LimitBasis);
        Assert.Equal("crypto", marketAdvertisements[0].LimitBasisText);
        Assert.Equal("10001", Assert.Single(marketAdvertisements[0].TradeMethods).PaymentId);
