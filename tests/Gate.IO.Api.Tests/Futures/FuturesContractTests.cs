...
public class FuturesContractTests
{
    ...
    public void Documented_futures_public_market_responses_deserialize()
    {
        ...
        Assert.Equal(0.95m, stats[0].LongShortTakerRatio);
        Assert.Equal(1.05m, stats[0].LongShortPositionUserRatio);
        Assert.Equal(4560.25m, stats[0].LongLiquidationUsdNew);
        Assert.Equal(3040.125m, stats[0].ShortLiquidationUsdNew);
        Assert.Equal(57756869.5m, stats[0].TopLongSize);
        Assert.Equal(30235536.25m, stats[0].TopShortSize);
        Assert.Equal(7776600.75m, stats[0].LongTakerSize);
        Assert.Equal(8579935.125m, stats[0].ShortTakerSize);
        Assert.Equal(50, stats[0].TopLongAccount);
        Assert.Equal(76, stats[0].TopShortAccount);
    ...
    public void Documented_futures_private_account_and_order_responses_deserialize()
    {
        ...
        Assert.Equal(10000L, position.UserId);
        Assert.Equal("BTC_USDT", position.Contract);
        Assert.Equal(-9440m, position.Size);
        Assert.Equal(GateFuturesHedgeStatus.PartiallyHedged, position.HedgeStatus);
        Assert.Equal(12m, position.HedgedSize);
        Assert.Equal(9428m, position.UnhedgedSize);
        Assert.Equal(0m, position.Leverage);
        Assert.Equal(100m, position.RiskLimit);
        Assert.Equal(100m, position.LeverageMax);
        Assert.Equal(0.005m, position.MaintenanceRate);
        Assert.Equal(3568.62m, position.Value);
        Assert.Equal(4.431548146258m, position.Margin);
        Assert.Equal(3779.55m, position.EntryPrice);
        Assert.Equal(99999999m, position.LiquidationPrice);
        Assert.Equal(3780.32m, position.MarkPrice);
        Assert.Equal(17.8431m, position.InitialMargin);
        Assert.Equal(1.78431m, position.MaintenanceMargin);
        Assert.Equal(-0.000507486844m, position.UnrealisedPnl);
        Assert.Equal(0.045543982432m, position.RealisedPnl);
        Assert.Equal(0.045543982432m, position.PnlPnl);
        Assert.Equal(0m, position.PnlFund);
        Assert.Equal(0m, position.PnlFee);
        Assert.Equal(0m, position.HistoryPnl);
        Assert.Equal(0m, position.LastClosePnl);
        Assert.Equal(0m, position.RealisedPoint);
        Assert.Equal(0m, position.HistoryPoint);
        Assert.Equal(5, position.AdlRanking);
        Assert.Equal(16, position.PendingOrders);
        Assert.NotNull(position.CloseOrder);
        Assert.Equal(232323L, position.CloseOrder.OrderId);
        Assert.Equal(3779m, position.CloseOrder.Price);
        Assert.False(position.CloseOrder.IsLiquidated);
        Assert.Equal(GateFuturesPositionMode.Single, position.Mode);
        Assert.Equal(0m, position.CrossLeverageLimit);
        Assert.Equal(new DateTime(2023, 5, 25, 6, 0, 6, DateTimeKind.Utc), position.UpdateTime);
        Assert.Equal(1L, position.UpdateId);
        Assert.Equal(new DateTime(2023, 5, 24, 6, 0, 0, DateTimeKind.Utc), position.OpenTime);
        Assert.Equal("BIG_HOT_COIN_50X_V2", position.RiskLimitTable);
        Assert.Equal(0.005m, position.AverageMaintenanceRate);
        Assert.Equal(12345L, position.PID);
        Assert.Equal(GateFuturesPositionMarginMode.Isolated, position.PositionMarginMode);
        Assert.Equal(30m, position.Lever);
